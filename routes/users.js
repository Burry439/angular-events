...
router.get('/getattendingevents',(req,res)=>{
    console.log(">"+req.headers.authorization)
    User.findById(req.headers.authorization).populate('attending', 'location name details date time image attending host').exec((err,events)=>{
        console.log(">:"+events+":<")
        res.json(events)
    })
})
...
router.get('/getcreatedevents',(req,res)=>{
    console.log(">"+req.headers.authorization)
    User.findById(req.headers.authorization).populate('events', 'location name details date image time attending').exec((err,events)=>{
        console.log(">"+events+"<")
        res.json(events)
    })
})
